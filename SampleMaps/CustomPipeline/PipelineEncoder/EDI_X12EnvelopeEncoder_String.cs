...
namespace SSPS.BizTalk.PipelineComponents
{
        ...
        IComponentUI
    {
        ...
        public Stream ReformatEnvelope(Stream inStream)
        {
            ...
            sDocument = sr.ReadToEnd().ToString();
            sr.Close();

            string detailLine = null;
            if (sDocument.Length > 105)
            {
                detailLine = sDocument.Substring(0, 102);
                detailLine = detailLine + "T";
                detailLine = detailLine + sDocument.Substring(103);
                if (detailLine != null)
                {
                    sw.Write(detailLine);
                    sw.Flush();
